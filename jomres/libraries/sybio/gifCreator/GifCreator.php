...
class GifCreator
	...
	public function __construct()
		...
		$this->errors  = array ( 'ERR00' => 'Does not supported function for only one image.', 'ERR01' => 'Source is not a GIF image.', 'ERR02' => 'You have to give resource image variables, image URL or image binary sources in $frames array.', 'ERR03' => 'Does not make animation from animated GIF source.', );
		}
	...
	public function addGifFrames( $i, $d )
		...
		if ( $this->colour > -1 && ord( $this->frameSources[ $i ]{10} ) & 0x80 )
			...
			for ( $j = 0; $j < ( 2 << ( ord( $this->frameSources[ $i ]{10} ) & 0x07 ) ); $j++ )
				...
				if ( ord( $Locals_rgb{3 * $j + 0} ) == ( ( $this->colour >> 16 ) & 0xFF ) && ord( $Locals_rgb{3 * $j + 1} ) == ( ( $this->colour >> 8 ) & 0xFF ) && ord( $Locals_rgb{3 * $j + 2} ) == ( ( $this->colour >> 0 ) & 0xFF )
				)
					{
